...
if(Moderator::loggedin()===false) {
	...
	if(Input::exists()) {
		if(Token::check(Input::get('token'))) {
			...
			if($validation->passed()) {
				...
				$db = DB::getInstance()->insert('coaches',array(
					'country' => $country,							 
					'name' => Input::get('name'),							 
					'born' => Input::get('born'),							 
					'dan' => Input::get('dan'),
					...
					'certif' => Input::get('certif'),
					'eurat' => Input::get('eurat'),
					'picture' => $slika
				));
				Session::flash("success", "New Coach successfully registered.");			
			} else {
	...
	echo "<article class='mb'>
		<div class='header'>
			<span class='title'>Coaches registry for country ". $country ."</span>
			...
			<form action='' method='post' enctype='multipart/form-data'>";
				...
				echo "<div class='dinline mr'>
					...
					<br/><input type='text' name='diploma' class='input ta-c mb-10' onkeydown=\"return ( event.ctrlKey || event.altKey || (47<event.keyCode && event.keyCode<58 && event.shiftKey==false) || (95<event.keyCode && event.keyCode<106) || (event.keyCode==8) || (event.keyCode==9) || (event.keyCode>34 && event.keyCode<40) || (event.keyCode==46) )\">
				</div><div class='dinline'>
					<label>Certificate ID</label>
					<br/><input type='text' name='certif' class='input ta-c mb-10' onkeydown=\"return ( event.ctrlKey || event.altKey || (47<event.keyCode && event.keyCode<58 && event.shiftKey==false) || (95<event.keyCode && event.keyCode<106) || (event.keyCode==8) || (event.keyCode==9) || (event.keyCode>34 && event.keyCode<40) || (event.keyCode==46) )\">
				</div>
				<div><label>Eurat level</label>
				<br/><input type='text' name='eurat' class='input ta-c mb-10'></div>
	...
	echo "<article class='mb'>
		<div class='header'>
			<span class='title'>Registered Coaches for country ". $country ."</span>
		</div><div class='list ta-c' aling='center'>";
		$blackbelts = new Coaches($country);
		if($blackbelts->count()!=0) {
			foreach($blackbelts->get() as $bb) {
				echo "<div class='li'><span>{$bb->name}</span>
					<div class='buttons'><!--<i class='edit'></i>--><i class='delete' onClick=\"deleteItem('{$bb->id}','3')\"></i></div>
				</div>";
			}
		} else {
			echo "<div class='notify mt-10'><div class='icon'></div><div class='text'>There are no registered Coaches for your country.</div></div>";
		}
		echo "</div>
	</article>";
}
