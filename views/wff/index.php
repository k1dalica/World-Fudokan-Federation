...
if(Moderator::loggedin()===false) {
	...
	if(Input::exists()) {
		if(Token::check(Input::get('token'))) {
			...
			if($validation->passed()) {
				...
				$db = DB::getInstance()->insert('blackbelts',array(
					'country' => $country,							 
					'name' => Input::get('name'),							 
					'born' => Input::get('born'),							 
					'dan' => Input::get('dan'),
				...
				Session::flash("success", "New Black Belt successfully registered.");			
			} else {
	...
	echo "<article class='mb'>
		<div class='header'>
			<span class='title'>Black Belts registry for country ". $country ."</span>
	...
	echo "<article class='mb'>
		<div class='header'>
			<span class='title'>Registered Black Belts for country ". $country ."</span>
		</div><div class='list ta-c' aling='center'>";
		$blackbelts = new Blackbelts($country);
		if($blackbelts->count()!=0) {
			foreach($blackbelts->get() as $bb) {
				echo "<div class='li'><span>{$bb->name}</span>
					<div class='buttons'><!--<i class='edit'></i>--><i class='delete' onClick=\"deleteItem('{$bb->id}','1')\"></i></div>
				</div>";
			}
		} else {
			echo "<div class='notify mt-10'><div class='icon'></div><div class='text'>There are no registered Black Belts for your country.</div></div>";
		}
		echo "</div>
	</article>";
}
